...
import util, key, byte_util
from byte_util import Error, ParseError, fmt_w_size, splitn, parse
...
class Message(object):
    ...
    def parse(cls, bytes, header=None):
        ...
        gen_log.debug('Parsing message %s' % (header.command,))
...
class FastBlock(Message):
    COMMAND = 'fastblock'
    HASH_FMT = fmt_w_size('<32s')

    HDR_FMT = fmt_w_size('<I32s32sIII')

    def __init__(self, version, prev_block_hash, merkle_root, timestamp, bits, nonce, tx_hashes, header=None):
        super(FastBlock, self).__init__(header=header)
        self.version = version
    ...
    def payload(self):
        return ''.join(
            [self.block_header,
             encode_varint(len(self.tx_hashes)),
             ''.join(self.tx_hashes)])
    ...
    def parse(cls, bytes, header=None):
        ...
        for _ in xrange(num_tx):
            (tx_hash, bytes) = parse(bytes, cls.HASH_FMT)
            tx_hashes.append(tx_hash[0])
        return (cls(version, prev_block_hash, merkle_root, timestamp, bits, nonce, tx_hashes), bytes)
    
    def build(self):
        return Block(self.version, self.prev_block_hash, self.merkle_root, self.timestamp, self.bits, self.nonce, [self.txns.get(tx_hash) for tx_hash in self.tx_hashes])
    ...
    def __repr__(self):
        return 'Block(%u txns, %r, %s, %s, %r, %r, %r, %r)' % (
            len(self.txns),
            self.version,
...
class Block(Message):
    ...
    def cut(self):
        return FastBlock(self.version, self.prev_block_hash, self.merkle_root, self.timestamp, self.bits, self.nonce, [tx.tx_hash for tx in self.txns])
